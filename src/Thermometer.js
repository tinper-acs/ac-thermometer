...
class Thermometer extends Component {
  ...
  _generateOptions() {
    return {
      ...
      percent: () => (this.options.value - this.options.min) / (this.options.max - this.options.min) * 100,
      intervals: []
    }
  }
  ...
  _createIntervals() {
    if (this.options.steps) {
      for (let step = 0; step <= this.options.steps; step++) {
        ...
        let interval = { percent: percent, label: +val + this.options.min + this.options.format }
